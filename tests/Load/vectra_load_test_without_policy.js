...
// K6 options
//
// Single ramping-arrival-rate scenario:
//   Stage 1 - ramp from 0 -> TARGET_RPS over RAMP_DURATION
//   Stage 2 - hold TARGET_RPS for STEADY_DURATION
// ---------------------------------------------------------------------------
export const options = {
  ...
  thresholds: {
    ...
    // Fail immediately if the server goes down (connection refused)
...
export function setup() {
  ...
  // Pre-flight: verify the server is reachable before registering agents
  console.log(`[setup] Probing ${BASE_URL}/health ...`);
  const probe = http.get(`${BASE_URL}/health`, { timeout: "15s" });
  if (probe.error_code !== 0 || (probe.status !== 200 && probe.status !== 204)) {
    throw new Error(
  ...
  console.log(`[setup] Registering ${POOL_SIZE} pooled agents against ${BASE_URL} ...`);
  ...
  for (let i = 0; i < POOL_SIZE; i++) {
    ...
    const regRes = http.post(
      `${BASE_URL}/agents`,
      JSON.stringify({ name, ownerId: OWNER_ID, clientSecret: secret }),
      { headers: { "Content-Type": "application/json" }, timeout: "15s" }
    );

    if (regRes.status !== 200 && regRes.status !== 201) {
      agentRegErrors.add(1);
      console.error(`[setup] Registration FAILED (HTTP ${regRes.status}) agent[${i}] -- ${regRes.body}`);
      continue;
    }
    ...
    try {
      ...
      if (!registeredId) throw new Error("AgentId missing");
    } catch (e) {
      console.error(`[setup] Cannot read AgentId for agent[${i}]: ${e}  body=${regRes.body}`);
      continue;
    }
    ...
    const tokenRes = http.post(
      `${BASE_URL}/tokens`,
      JSON.stringify({ agentId: registeredId, clientSecret: secret }),
      { headers: { "Content-Type": "application/json" }, timeout: "15s" }
    );
    ...
    if (tokenRes.status !== 200) {
      authErrors.add(1);
      console.error(`[setup] Token FAILED (HTTP ${tokenRes.status}) agent=${registeredId} -- ${tokenRes.body}`);
      continue;
    }
    ...
    try {
      ...
      console.error(`[setup] Cannot read AccessToken for agent ${registeredId}: ${e}  body=${tokenRes.body}`);
      continue;
    }
    ...
    const policyRes = http.put(
      ...
      JSON.stringify({ policyName: "Public or Admin Access" }),
      { headers: { "Content-Type": "application/json" }, timeout: "15s" }
    );
    if (policyRes.status !== 200 && policyRes.status !== 204) {
      console.error(`[setup] Policy assign FAILED (HTTP ${policyRes.status}) agent=${registeredId} -- ${policyRes.body}`);
      // continue anyway — proxy calls will 500 but other endpoints still work
    }

    agents.push({ agentId: registeredId, token });
    console.log(`[setup] Agent ${i + 1}/${POOL_SIZE} ready (id=${registeredId})`);
  }

  if (agents.length === 0) {
    throw new Error(
      `[setup] FATAL: 0/${POOL_SIZE} agents provisioned. ` +
      `Verify ${BASE_URL} is reachable and POST /Agents + POST /Tokens return 200/201.`
    );
  }

  console.log(`[setup] Pool ready -- ${agents.length}/${POOL_SIZE} agents provisioned.`);
  return { agents };
}
...
// ---------------------------------------------------------------------------
// error_code 1212 = connection refused; 1210 = dial error (covers both TCP reset and refused)
...
export default function (data) {
  ...
  if (roll < 0.70) {
    ...
    // URL must NOT be percent-encoded: ASP.NET Core does not decode %2F in
    // Request.Path, so Uri.TryCreate would receive "https:%2F%2F..." and fail.
      const res = http.get(`${BASE_URL}/proxy/http://localhost:7080/health`, {
      headers: authHeader,
      timeout: "15s",
    });
    ...
    if (CONNECTION_REFUSED.includes(res.error_code)) {
      ...
      console.error(`[VU ${__VU}] Server refused connection on /proxy — is Vectra still running?`);
      return;
    }
    ...
    if (!ok) proxyErrors.add(1);

  } else if (roll < 0.90) {
    // 20 % - List agents
    const res = http.get(`${BASE_URL}/agents?page=1&pageSize=25`, {
      headers: authHeader,
      timeout: "15s",
    });
    ...
    if (CONNECTION_REFUSED.includes(res.error_code)) {
      serverDownErrs.add(1);
      console.error(`[VU ${__VU}] Server refused connection on /agents — is Vectra still running?`);
      return;
    }
    ...
    if (CONNECTION_REFUSED.includes(res.error_code)) {
      serverDownErrs.add(1);
      console.error(`[VU ${__VU}] Server refused connection on /hitls — is Vectra still running?`);
      return;
    }

    const ok = check(res, {
      "hitl 200 or 204": (r) => r.status === 200 || r.status === 204,
    });
...
export function teardown(data) {
  const count = data && data.agents ? data.agents.length : 0;
  console.log(`[teardown] Done. Pool size used: ${count} agents.`);
}
